...
We want to introduce the new Database Connection UI flow my team has been working within Preset Inc. Connecting Superset to your database(s) is the first thing an admin must do before  users can write queries and build charts/dashboards, yet it's one of the most painful tasks, requiring knowledge of SQLAlchemy. To elevate this experience we built a custom form where admins can choose their database and fill in a form with the parameters required for that connection, instead of asking them for a full SQLAlchemy URI. The previous flow is still available if needed.

**Add Giph for before and after**

There are now 3 steps when connecting to a database:
...
For copyright reasons the logos for each database are not distributed with Superset (does anyone know if we can hotlink to them?).

### Setting images

- To set the images of you preferred database admins must create a mapping in the `superset_text.yml` file with engine and location of the image. The image can be host locally in inside your static/file directory or online (e.g. S3)
