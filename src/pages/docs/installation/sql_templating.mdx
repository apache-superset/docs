...
SQL Lab supports [Jinja templating](https://jinja.palletsprojects.com/en/2.11.x/) in queries. You'll need to to overload the default Jinja context in your environment by defining the JINJA_CONTEXT_ADDONS in your superset configuration (`superset_config.py`). Objects referenced in this dictionary are made available for users to use in their SQL code.
...
The available validators and names can be found in [sql_validators](https://github.com/apache/incubator-superset/tree/master/superset/sql_validators).
